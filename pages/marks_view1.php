	...
	xmlhttp.open("GET","getclassid_marks1.php?year="+str,true);
	xmlhttp.send();
	}
	...
	xmlhttp.open("GET","getexamtype_marks1.php?classid="+str+"&year="+str1,true);
	xmlhttp.send();
	}
        ...
        <table >
            ...
            <tr>
                ...
                <td><select name="sid1" id="textbox" />
                    <option></option>
                    <?php
                        $sql= "select DISTINCT SID from marks_2011 order by SID ASC";
                        $query=mysql_query($sql);
                        while($row=mysql_fetch_assoc($query))
                        {
...
<?php
	...
	{ 
		$sql= "select * from marks_2011 where MID=".$id;
		$query=mysql_query($sql);
		while($row=mysql_fetch_assoc($query))
		{
...
<?PHP 
	...
	{
		$sql= "update `marks_2011` set `Obt_Theory` = '".$obt_theory."' , `Obt_Practical` = '".$obt_practical."' , `Obt_Total` = '".$obt_total."' where MID='".$id."'";
	...
	{ 	
		$sql= "delete from `marks_2011` where MID='".$id."'";
		$result=mysql_query($sql);
		if ($result=='1'){
			echo "Marks Deleted";
		}else{
...
?>
        ...
        <?php
         ...
         ?>
            ...
            <tr>
                <td>Year</td>
                <td><input type="text" name="year" size="10" value= "<?php echo $year1; ?>" readonly="readonly" /></td> 
            </tr>
    
            <tr>
                <td>Class ID</td>
                <td><input type="text" name="classid" size="10" value= "<?php echo $classid1; ?> " readonly="readonly"/></td>
            </tr>
    
            <tr>
                <td>Exam Type </td>
                <td><input type="text" name="exam_type" size="10" value="<?php echo $exam_type1; ?>" readonly="readonly"/></td>
            </tr>
    
            <tr>
                <td>Student ID </td>
                <td><input type="text" name="sid" size="10" value="<?php echo $sid1; ?>" readonly="readonly" /></td>
            </tr>
        ...
        ?>
            <tr>
                <td><a href="main.php?page=marks_view&action=edit_marks&id= <?PHP echo $row['MID']; ?>"><img src="../images/edit.png" /></td> <!--for edit-->
                <td><a href="main.php?page=marks_view&action=del_marks&id= <?PHP echo $row['MID']; ?>" onclick="return confirm_delete()"><img src="../images/delete.png" /></td> <!-- for delete-->
        ...
        <?PHP
				if($row['Obt_Theory']<32 or $row['Obt_Practical']<8) // To check whether pass or fil
					$result_counter=1;
				...
				{		$percent=($total_counter/$counter)*100;
						...
						elseif($percent<60 and $percent>=50)
							echo "<tr><td colspan=9>Division : Second</td></tr>";
						else
							echo "<tr><td colspan=9>Division : Third</td></tr>";
